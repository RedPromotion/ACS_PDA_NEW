...
*******************************************************************************************/
/*******************************************************************************************	
@Page pageTransferSkin.jsx
@Role Motras PDA > 출하 메뉴 > 스킨 이관 페이지
@description 이전 명칭 frmTransferSkin.cs  , [EAI].[dbo].[IRTSKIN] 테이블 조회 및 저장 함. (천안공장)
*******************************************************************************************/
export default function PageTransferSkin() {

    const { setPopup, setOnLoading, setDecision, isTopBarVisible, setIsTopBarVisible, } = useRootContext();

    const scanLocationRef = useRef('transferSkinBarcode'); //네이티브에서 기준삼을 스캔 위치 플래그 값  (매개변수에 처음 지정위치 포함)

	const [barcode, setBarcode] = useState('');//바코드
	const focuseBarcode = useRef(null);//팔레트 텍스트필드 포커스 기준	  
		
	const [gridSkinTransfer, setGridSkinTransfer] = useState({//이관 팔레트 내용 데이터그리드
		columns : [			
			{ field: 'skin_barcode', headerName: '바코드', width: 100 },
			{ field: 'part_no', headerName: '부품 번호', width: 100 },
			{ field: 'PLT_NO', headerName: '팔레트 번호', width: 100 },
			{ field: 'prod_date', headerName: '생산일자', width: 100 },	
		],
		rows : [],
	});
	...
	const gridSkinTransfer_Ref = useRef(''); 
	const count_Ref = useRef(0); 
	useEffect(() => {  barcode_Ref.current = barcode }, [barcode]);
	useEffect(() => {  gridSkinTransfer_Ref.current = gridSkinTransfer }, [gridSkinTransfer]);
	...
	const nativeReadData = (e) => {
		...
		if (type === 'SCANDATA') {			
			...
			try{
				if (scanLocationRef.current === 'transferSkinBarcode') {
	...
	const onFocuseBarcode = () => {
		scanLocationRef.current = 'transferSkinBarcode'
	...
	const onKeydownForEvent = (event) => {
        ...
        if(event.key === 'Enter' &&  scanLocationRef.current === 'transferSkinBarcode'){
		...
		(async () => {
			try {
				const result = await UseFetchParam({
					procedure:"PSP_L_SKIN_TRANSFER", //천안공장 테이블 스킨 조회
					parameter:[ scannedBarcode ],
					EventType:"barcode read",
					EventName:"search skin Transfer data",
					isSelectMultiple:true,
				});
				if( result[0].length === 0 || !result ){ 
					throw new Error(`조회된 스킨이관 대상의 결과가 없습니다.`);
				}
				if ( ! scannedBarcode === result[0][0].PLT_NO  ) {
					throw new Error(`조회된 스킨이관 대상의 정보가 상이합니다. [${scannedBarcode} : ${result[0][0].PLT_NO}]`);
				}
	...
	const onTransferSkin = ()=> {  

		/** (내부함수) 발포이관 확정 버튼 클릭 이벤트 */
		const confirmSave = ()=>{			
			setOnLoading(true);	
			(async () => {
				try {
					const result = await UseFetchParam({
						procedure:'PSP_S_SKIN_TRANSFER', //발포이관 저장 진행
						parameter:[ barcode_Ref.current ],
						EventType:"save data",
						EventName:"save skin transfer data",
					});				
					//성공 알림
					setPopup(`입고처리 되었습니다.`);
					onReset();
		...
		if(count_Ref.current === 0 && barcode === ''){ //미입력 예외처리
		...
		if(count_Ref.current !== gridSkinTransfer_Ref.current.rows.length){// 갯수 불일치 예외처리 , 선택적 처리 제공 예외처
			setDecision({
				open: true,
				title: "추가 갯수 오류",
				text: "실제 갯수와 추가된 갯수가 일치하지 않습니다. 다시 입력하시겠습니까?",
				okText: "예", // 저장 진행 중지, 다시 입력 셋팅
				noText: "아니요", // 그냥 취소
				okEvent: () => { onReset() },
				noEvent: () => { return },
			})
		}	
		else if(count_Ref.current === gridSkinTransfer_Ref.current.rows.length){  // 갯수 일치, 바로 저장여부 묻고 진행
			setDecision({ //  저장 전 다시 확인하는 선택팝업
				open: true,
				title: "확인",
				text: "저장하시겠습니까",
				okText: "Yes",
    ...
    <Stack gap={1} m={1}>
		...
		<CatDataGrid 
			row={[...gridSkinTransfer.rows].reverse()}
			col={gridSkinTransfer.columns}
		/>		
