...
*******************************************************************************************/
/*******************************************************************************************	
@Page PageASShipment.jsx
@Role Motras PDA > 출하 메뉴 > AS출하 페이지
@description 이전 명칭 frmASChulha.cs
*******************************************************************************************/
export default function PageASShipment() {

    const { 
		setPopup, setOnLoading,
		isTopBarVisible, setIsTopBarVisible,
	} = useRootContext();
	const scanLocationRef = useRef('ASShipmentpalletBarcode'); //네이티브에서 기준삼을 스캔 위치 플래그 값  (매개변수에 처음 지정위치 포함)
	...
	* -----------------------------------------------------*/ 
	...
	const [DataGrid_AS, setDataGrid_AS]  = useState({ //AS출하 데이터그리드
		columns : [
	...
	const [ASSPOT, setASSPOT] = useState({selected: "", options: []});//출하처 셀렉트
	...
	//useRef에 값 복사
	const DataGrid_AS_ref = useRef(''); 
	useEffect(() => { DataGrid_AS_ref.current = DataGrid_AS  }, [DataGrid_AS]);	
	
	const ASSPOT_ref = useRef(''); 
	useEffect(() => { ASSPOT_ref.current = ASSPOT }, [ASSPOT]);	
			...
			try {
				...
				const result = await UseFetchParam({
					...
					parameter:[
						...
						(SELECT TRUCK_CODE 
						 FROM M_TCST(nolock) 
						 WHERE SPOT_CODE = ${ASSPOT.selected})`
					], 
					EventType:"load code",
	...
	const nativeReadData = (e) => {
		...
		if (type === 'SCANDATA') {			
			...
			try{
				if (scanLocationRef.current === 'ASShipmentpalletBarcode') {
    ...
    const onKeydownForEvent = (event) => {
        ...
        if(event.key === 'Enter' && scanLocationRef.current === 'ASShipmentpalletBarcode'){
	...
	const onFocuseBarcode = () => {
		scanLocationRef.current = 'ASShipmentpalletBarcode';
		if ( focuseBarcode.current ){
			focuseBarcode.current.value = ''
			focuseBarcode.current.focus(); 
		}
		if( document.getElementById('ASShipmentpalletBarcode') ){
            document.getElementById('ASShipmentpalletBarcode').value = '';
        }
	}
	...
	const handleBarcodeInput = (scannedBarcode) => {
		...
		if( DataGrid_AS_ref.current.rows.some( rows => rows.PalletNo === scannedBarcode )){
			setOnLoading(false);
		...
		(async () => {
			try {
				const result = await UseFetchParam({
					procedure:"PDA_ASCHULHA_BARCODE_CHECK_L",  //팔레트 체크 (대체: PDA_ASCHULHA_BARCODE_CHECK_L) (원본:PSP_L_ASCHULHA_BARCODE_CHECK)
					parameter:[ scannedBarcode ],
					EventType:"barcodeScan",
					EventName:"AS_shipment",
				});
				if(result[0].Column1 === "OK"){	
					setDataGrid_AS((prevDataGrid_AS) => ({
					...prevDataGrid_AS,
					rows: [
						...prevDataGrid_AS.rows,
						{ 
							id: scannedBarcode
							, PalletNo: scannedBarcode
							, PACKDATE: result[0].Column2 
						}
					]
					}))
				}
				focuseBarcode.current = ''
			}
			catch (err) {
				setPopup(`${err}`)
			} 
	...
	const onPageRagistDetail = () => {
		if(  DataGrid_AS_ref.current.rows.length === 0 ){
	...
	const onSave = () => {
		...
		if( !ASSPOT_ref.current.selected ){						
		...
		(async () => {
			try {
				const result1 = await UseFetchParam({					
					...
					EventType:"CREATE ERP NUM",
					EventName:"create Shipment AS ERP code",					
				});	
				// 출하번호(ERP번호) 안 나올 시 예외처리
				if( !result1[0].Column1 ){ //출하번호(ERP번호) 안 나올 시 예외처리	
				...
				filteredRows_ref.current = DataGrid_AS_ref.current.rows //기존 팔레트 복사본
				...
				//반복하여 팔레트 처리
				for ( const row of DataGrid_AS_ref.current.rows ) {
					try {
						const result2 = await UseFetchParam({
							procedure:"CSP_DBSave_ASPACK",
							parameter:[ 
								row.PalletNo  , // 팔레트 코드
								ASSPOT.selected, 
								pageFlag === 'registDetail' ? textTransport_ref.current : deliveryTransport_ref.current.selected, //트럭코드(truck_code)
								pageFlag === 'registDetail' ? textDriver_ref.current : "0" ,  //운전기사(driver)
								pageFlag === 'registDetail' ? Number(textCost_ref.current) : "0" , //운송가격(truck_cost)
								result1[0].Column1 //출하번호 ERP 앞의 프로시저에서 받아옴
							], 
							EventType:"Save Data",
							EventName:"save AS shipment pallet",
				...
				if (faildBarcode_ref.current.length === 0){ // 전체성공, 실패한 경우가 없는 경우
					setPopup('AS출하 성공',`전체 출하처리 되었습니다.`);
				}					
				else if (faildBarcode_ref.current.length > 0 && savedBarcode_ref.current.length > 0 ) { // 일부실패
					setPopup('일부 AS출하 실패'`일부 팔레트가 출하되지 않았습니다. [성공${savedBarcode_ref.current.length} / 실패${faildBarcode_ref.current.length}] [에러:${faileMsgSave_ref.current}]`);
				}
				else if (savedBarcode_ref.current.length === 0) { // 전체실패
					setPopup('AS출하실패'`바코드 전체가 출하되지 않았습니다. [${faileMsgSave_ref.current}]`);
				}
				
				// 저장 실패한 바코드만 데이터그리드에 남김 (없을 경우 공백)
				setDataGrid_AS((prevState) => ({
					...prevState, // 이전 상태 복사					
    ...
    <Stack gap={1} m={1}>
		...
		<>							
            ...
            <CatDataGrid
				row={[...DataGrid_AS.rows].reverse()} 
				col={DataGrid_AS.columns} 
                rowSelectionModel={DataGrid_AS.selected}
			...
			<CatButtonBigOne 
				buttonLabel='선택 삭제'
				onClick={() => {
					const selectionModel = DataGrid_AS_ref.current.selected;                    
                    //선택된 항목 없으면
                    if (!selectionModel.ids || selectionModel.ids.size === 0) {
                        setPopup('삭제 실패', '선택된 삭제 대상이 없습니다.');
                        return;
                    }                    
                    setDataGrid_AS(prevState => ({//삭제: Set.has()로 필터링
                        ...prevState,
                        rows: prevState.rows.filter(row => !selectionModel.ids.has(row.id)),
                        //선택 모델 초기화 (빈 Set)
                        selected: {
                            type: 'include',
                            ids: new Set(),
		...
		<>
			<CatSelect
				label="출하처"
				value={ASSPOT.selected} 
				option={ASSPOT.options}
				onChange={(event) => {
					setASSPOT({
						...ASSPOT,
